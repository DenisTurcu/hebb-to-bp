...
def plot_and_fit(
    X, Y, ax=None, color=None, marker=".", linestyle="--", label="", xlabel="", title=""
):
    if ax is None:
        _, ax = plt.subplots()
    if color is None:
...
def plot_hidden_activity(h, isFam, out, ax=None):
    ...
    h = interleave(
        h, torch.full(h.shape, float("nan"))
    ).T  # insert col of nan's in between each col to force whitespace
    ...
    xticklabels = [
        xticklabels[i // 2] if i % 2 == 0 else "" for i in range(2 * len(xticklabels))
    ]
...
def plot_distribution(data, isFam, mean=False, xlabel="x", ax=None, bins=50):
    ...
    ax.plot([], color=patches[0].get_edgecolor(), label=r"p($\cdot$|y=0)")
    ...
    ax.plot([], color=patches[0].get_edgecolor(), label=r"p($\cdot$|y=1)")
    if np.abs(bins - bins.mean()).sum() < 0.01:
        ax.set_xlim(bins.mean() - 0.01, bins.mean() + 0.01)
    ...
    if mean:
        ...
        ax.axvline(
            data[isFam == 1].mean(), linestyle="--", linewidth=0.5, color="green"
        )
    ...
    try:
        ...
        for decoder in acc.keys():
            ...
            for i, popSize in enumerate(popSizeList):
                ...
                w2, b2 = compute_FLD_weights(
                    h, isFam, neuronsMask=neuronsMask, SCC=(decoder == "SCC")
                )
                net.w2, net.b2 = nn.Parameter(
                    torch.tensor(w2, dtype=torch.float).unsqueeze(0)
                ), nn.Parameter(torch.tensor(b2, dtype=torch.float))
...
def compute_FLD_weights(h, isFam, neuronsMask=None, SCC=False, dPrime=False):
    ...
    if SCC:
        ...
        Sinv = np.linalg.inv(S) if not np.isscalar(S) else np.array([1.0 / S])
        w = np.zeros(N)
        w[neuronsMask] = -np.matmul(Sinv, mN - mF)

    b = -np.dot(w[neuronsMask], (mN + mF) / 2.0)
    return w, b
...
def get_corr_Wxb_Ax(fname, net=None, Rs=None):
    ...
    try:
        ...
        corr = {
            nf: {stat: np.zeros(len(Rs)) for stat in ["mean", "std"]}
        ...
        for i, R in enumerate(Rs):
            ...
            data = generate_recog_data(
                T=max(R * 20, 5000), d=d, R=R, P=0.5, multiRep=False
            )
...
def fit_idealized(R, Pfp, Ptp, D, n, f=2.0 / 3):
    N = 2**n
    ...
    def fit_me(R2, a0, b):
        ...
        while True:
            fEff = (1 - PfpOld) * f + (1 - PtpOld) * (
                1 - f
            )  # fraction of items *reported* as novel
            a = a0 * np.sqrt(f / fEff)
            ...
            if np.all(np.abs(PfpOld - Pfp) < 0.0001) and np.all(
                np.abs(PtpOld - Ptp) < 0.0001
            ):
                break
    ...
    (aOpt, bOpt), _ = optimize.curve_fit(
        fit_me,
        xdata=np.concatenate([R, R]),
        ydata=np.concatenate([Pfp, Ptp]),
        p0=[aInit, bInit],
...
def get_generalization(
    ...
    try:  # load result if it exists
        res = joblib.load(resultSaveFile)
        testR, testAcc, truePosRate, falsePosRate = (
            res["testR"],
            res["testAcc"],
            res["truePosRate"],
            res["falsePosRate"],
        ...
        if net is None:
            net = load_from_file(fname)
        if gen_data is None:
            gen_data = lambda R: generate_recog_data(
                T=max(T, R * 20),
                R=R,
                d=d,
                P=0.5 if not multiRep else 1.0 / 3,
                multiRep=multiRep,
            )
        testR, testAcc, truePosRate, falsePosRate = get_recog_positive_rates(
            net, gen_data, xscale=xscale, upToR=upToR, stopAtR=stopAtR
        )
        res = {
            "testR": testR,
            "testAcc": testAcc,
...
def get_recog_positive_rates(
    net, gen_recog_data, xscale="log", upToR=float("-inf"), stopAtR=float("inf")
):
    testR = []
    testAcc = []
    truePosRate = []
    ...
    while (truePos > falsePos or R < upToR) and R < stopAtR:
        ...
        testR.append(R)
        print(
            "R={}, truePos={:.3f}, falsePos={:.3f}, acc={:.3f}={:.3f}, (chance={:.3f})".format(
                R, truePos, falsePos, acc, acc2, chance
            )
        )
        if xscale == "log":
            R = int(np.ceil(R * 1.3))
