...
def generateWordBank(toy_params):
    ...
    variable_length = (
        toy_params["variable_length"] if "variable_length" in toy_params else False
    )
...
def scoreToyPhrase(toy_phrase, toy_params):
    ...
    for idx in range(phrase_length):
        ...
        if not_length > 0:
            not_length -= 1  # decays not
        if extreme_length > 0:
            extreme_length -= 1  # decays extremely
...
def classifySentiment(score, toy_params):
    ...
    if outer_prod:
        ...
        sentiment_tensor = np.array(
            [sentiment_tensor_final for idx in range(phrase_length)]
        )
        ...
        if len(toy_params["words"]) == 3:
            ...
            for class_idx in range(1, n_sentiments):
                sub_divs.append(
                    -1 * phrase_length + class_idx * score_range / n_sentiments
                )
        elif len(toy_params["words"]) == 5:
            score_range = 4 * phrase_length + 1
            for class_idx in range(1, n_sentiments):
                sub_divs.append(
                    -2 * phrase_length + class_idx * score_range / n_sentiments
                )
        ...
        if loss_type == "XE":
            if n_sentiments == 2:
                ...
                raise NotImplementedError(
                    "n_sentiments only implemented for 2, 3, or 5 class"
                )
            sentiment_tensor_final = np.zeros((n_outputs,))
            sentiment_tensor_final[sentiments.index(sentiment)] = 1.0
            # This is a hack that just sets all times of sentiment tensor equal to final
            ...
            sentiment_tensor = np.array(
                [
                    [0.0] if idx != phrase_length - 1 else [score]
                    for idx in range(phrase_length)
                ]
            )
    ...
    for word_idx, word in enumerate(toy_params["words"]):
        ...
        while not vector_found:
            if toy_params["input_type"] in ("binary", "binary1-1", "binary_no_norm"):
                ...
                raise ValueError(
                    "Input type {} not recognized".format(toy_params["input_type"])
                )
            ...
            for word_idx2 in range(word_idx):
                if (
                    candidate_vec
                    == word_to_input_vector[toy_params["words"][word_idx2]]
                ).all():
                    repeat_found = True
...
def randomTrainingExample(
    ...
    if variable_length:
        min_phrase_len = toy_params["min_phrase_len"]
        phrase_length = min_phrase_len + np.random.randint(
            max_phrase_length - min_phrase_len
        )
    else:
    ...
    if (
        uniform_score and not ordered_class
    ):  # Uniform score generation for unordered classes
        ...
        uniform_score and ordered_class
    ):  # Uniform score generation for ordered classes
        if len(words) == 3:  # good, bad, the case
            ...
            phrase = score_to_phrase_ordered_general(
                score, score_to_idx_map, toy_params
            )
    ...
    if toy_params["n_delay"] > 0:  # Insert delay at end (before <eos> though)
        ...
        phrase[0:0] = [
            toy_params["delay_word"] for _ in range(abs(toy_params["n_delay"]))
        ]
    if toy_params["include_eos"]:
        phrase.append("<eos>")
...
def generate_data(
    ...
    if "loss_type" not in toy_params:
        toy_params["loss_type"] = "XE"
    if "ordered_classes" not in toy_params:
        toy_params["ordered_classes"] = False
    if "uniform_score" not in toy_params:
        toy_params["uniform_score"] = False
    if toy_params["uniform_score"]:
        if "eliminate_ties" not in toy_params:
            toy_params["eliminate_ties"] = True
    else:  # Eliminate ties not yet implemented for non-uniform scores
        if "eliminate_ties" not in toy_params:
            toy_params["eliminate_ties"] = False
    if "filter_classes" not in toy_params:
        toy_params["filter_classes"] = False
    if "input_type" not in toy_params:
        toy_params["input_type"] = "one hot"
    if "noise_var" not in toy_params:
        toy_params["noise_var"] = None
    # Optional ability to stack multiple phrases end to end
    if "stack_phrases" not in toy_params:
        toy_params["stack_phrases"] = False
    if "n_stack" not in toy_params:
        toy_params["n_stack"] = 1
    # Special characters at start or end of phrase
    if "include_eos" not in toy_params:
        toy_params["include_eos"] = False
    if "include_sos" not in toy_params:
        toy_params["include_sos"] = False
    # Insertion of delay period
    if "n_delay" not in toy_params:
        toy_params["n_delay"] = 0
    if "delay_word" not in toy_params:
        toy_params["delay_word"] = "<delay>"  # '<delay>' or 'null'

    if "context_words" not in toy_params:
        toy_params["context_words"] = None
    if "outer_prod" not in toy_params:
        toy_params["outer_prod"] = False
    if "variable_length" not in toy_params:
        toy_params["variable_length"] = False
    if "min_phrase_len" not in toy_params:
        toy_params["min_phrase_len"] = 0
    if "label_shift" not in toy_params:
        toy_params["label_shift"] = None

    if "words" not in toy_params:
        toy_params["words"] = generateWordBank(toy_params)
    if (
        "base_word_vals" not in toy_params
    ):  # Values of each words in terms of their scores
        toy_params["base_word_vals"] = baseWordValues(toy_params)
    if (
        "word_to_input_vector" not in toy_params
    ):  # Creates a unique input vector for each word
        if toy_params["input_type"] in (
            "one_hot",
            "binary",
            "binary1-1",
            "binary_no_norm",
            ...
            if "<delay>" in toy_params["words"]:  # Make the delay input vector smaller
                if "delay_scale" not in toy_params:
                    toy_params["delay_scale"] = 0.0
                toy_params["word_to_input_vector"]["<delay>"] = (
                    toy_params["delay_scale"]
                    * toy_params["word_to_input_vector"]["<delay>"]
                )
        else:
            raise ValueError("Input type not recognized!")

    if "input_size" not in toy_params:
        toy_params["input_size"] = len(toy_params["words"])
    ...
    if "classes" not in toy_params:
        if toy_params["ordered_classes"]:
            ...
            toy_params["classes"] = [
                "class" + str(i) for i in range(toy_params["n_classes"])
            ]
    ...
    if toy_params[
        "stack_phrases"
    ]:  # Modifies dataset size so it generates enough for stacking later
        n_stacked_datasets = np.copy(dataset_size)  # original number to be used later
        dataset_size = int(dataset_size * toy_params["n_stack"])

    syn_sentiments = []
    syn_phrases = []
    syn_inputs_np = np.zeros(
        (
            dataset_size,
            toy_params["phrase_length"],
            toy_params["input_size"],
        )
    )
    syn_targets_np = np.zeros(
        (
            dataset_size,
            toy_params["phrase_length"],
            out_size,
    ...
    if uniform_score and toy_params["ordered_classes"]:
        ...
        if len(toy_params["words"]) == 5 or len(toy_params["words"]) == 6:
            if "score_to_idx_map" not in toy_params:
                score_vals = (
                    toy_params["score_vals"]
                    if "score_vals" in toy_params
                    else [-2, -1, 1, 2]
                )
                toy_params["score_to_idx_map"] = generate_score_map(
                    len(score_vals), toy_params["phrase_length"], score_vals
                )
    elif uniform_score:  # Uniform score unordered
        check_word_bank_for_uniform_score(
            toy_params["words"], len(toy_params["classes"])
        )
    ...
    if auto_balance:  # Ensures the classes are balanced
        ...
        if filter_classes:
            ...
            num_per_class = np.ones((n_classes,)) * int(
                dataset_size / (n_classes - len(class_filter_idxs))
            )
        ...
        complete = False
        while not complete:
            sentiment, phrase, sentiment_tensor, phrase_tensor, target_mask = (
                randomTrainingExample(toy_params)
            )
            if (
                outer_prod
            ):  # Treats sentiment like a binary number and converts it into a class index
                class_idx = int(
                    np.sum(
                        [
                            sentiment_tensor[0, sent_idx] * (2**sent_idx)
                            for sent_idx in range(sentiment_tensor.shape[1])
                        ]
                    )
                )
            else:
            ...
            if class_count[class_idx] < num_per_class[class_idx]:
                ...
                if len(target_mask) > 1:
                    raise NotImplementedError
                syn_target_masks_np[trial, target_mask, :] = np.ones(
                    (out_size,), dtype=np.int32
                )
    ...
    if verbose:
        print(
            "Synthentic data generated in: {:0.2f} sec. Autobalanced: {}. Uniform score: {}. Eliminate ties: {}".format(
                time.time() - start_time,
                auto_balance,
                uniform_score,
                toy_params["eliminate_ties"],
            )
        )
    ...
    syn_targets_np = np.argmax(syn_targets_np, axis=-1)[
        :, :, np.newaxis
    ]  # Compresses last dimension to max argument
    ...
    if toy_params["stack_phrases"]:
        ...
        syn_inputs_stack = np.zeros(
            (
                n_stacked_datasets,
                phrase_len * toy_params["n_stack"],
                toy_params["input_size"],
            )
        )
        syn_targets_stack = np.zeros(
            (
                n_stacked_datasets,
                phrase_len * toy_params["n_stack"],
                1,
        ...
        syn_target_masks_stack = np.zeros(
            (
                n_stacked_datasets,
                phrase_len * toy_params["n_stack"],
                out_size,
        ...
        for data_idx in range(dataset_size):
            syn_inputs_stack[
                batch_count,
                stack_count * phrase_len : (stack_count + 1) * phrase_len,
                :,
            ] = syn_inputs_np[data_idx, :, :]
            syn_targets_stack[
                batch_count,
                stack_count * phrase_len : (stack_count + 1) * phrase_len,
                :,
            ] = syn_targets_np[data_idx, :, :]
            syn_target_masks_stack[
                batch_count,
                stack_count * phrase_len : (stack_count + 1) * phrase_len,
                :,
            ] = syn_target_masks_np[data_idx, :, :]
            stack_count += 1
    ...
    if create_tensors:  # Turns everything into Pytorch tensors, and creates a dataset
        ...
        syn_target_masks = torch.tensor(
            syn_target_masks_np, dtype=torch.bool, device=device
        )
...
def generate_special_data(toy_params, out_size, device="cpu"):
    ...
    for word_idx in range(len(word_list)):
        syn_input_np = np.zeros(
            (
                toy_params["phrase_length"],
                toy_params["input_size"],
            )
        )
        syn_target_np = np.zeros(
            (
                toy_params["phrase_length"],
                out_size,
            ),
        ...
        for seq_idx in range(toy_params["phrase_length"]):
            syn_input_np[seq_idx] = toy_params["word_to_input_vector"][
                word_list[word_idx]
            ]
    ...
    for word_idx in range(new_word_count):
        ...
        for seq_idx in range(toy_params["phrase_length"]):
            syn_input_np[seq_idx] = new_word_word_to_input[
                toy_params["words"][word_idx]
            ]
...
def generate_data_context(
    ...
    if "toy_params_1" not in toy_params:
        ...
        _, _, toy_params_1 = generate_data(
            1, toy_params_1, out_size, auto_balance=False, verbose=False
        )
        _, _, toy_params_2 = generate_data(
            1, toy_params_2, out_size, auto_balance=False, verbose=False
        )
        ...
        if not toy_params["include_eos"]:
            ...
            toy_params_2["word_to_input_vector"]["<eos>"] = toy_params_1[
                "word_to_input_vector"
            ]["<eos>"]
    ...
    inputs_1, targets_1, target_masks_1, raw_data_1, toy_params["toy_params_1"] = (
        generate_data(
            dataset_size,
            toy_params["toy_params_1"],
            out_size,
    ...
    inputs_2, targets_2, target_masks_2, raw_data_2, toy_params["toy_params_2"] = (
        generate_data(
            dataset_size,
            toy_params["toy_params_2"],
            out_size,
            auto_balance=auto_balance,
            verbose=False,
            create_tensors=False,
    ...
    if toy_params["input_type"] in ("binary", "binary1-1"):
        inputs_1 = (
            np.sqrt(toy_params["toy_params_1"]["input_size"] / toy_params["input_size"])
            * inputs_1
        )
        inputs_2 = (
            np.sqrt(toy_params["toy_params_2"]["input_size"] / toy_params["input_size"])
            * inputs_2
        )
    else:
        raise NotImplementedError()
    ...
    context_input = np.where(
        context_mask[:, np.newaxis],
    ...
    if toy_params["data_type"] == "retro_context_int":
        ...
        if toy_params["n_delay"] > 0:
            ...
            context_input[:, :pre_context, :] = np.zeros(
                (dataset_size, pre_context, context_input.shape[-1])
            )
            context_input[:, post_context:, :] = np.zeros(
                (
                    dataset_size,
                    toy_params["phrase_length"] - post_context,
                    context_input.shape[-1],
                )
            )
...
def generate_data_anti_context(
    dataset_size,
    toy_params,
    out_size,
    auto_balance=False,
    ...
    # If needed, does some initialization of the wrapped toy_params_1 (and _2)
    assert (
        toy_params["n_classes"] == 2
    )  # This could work for more classes, but not yet implemented
    ...
    if toy_params["data_type"] == "retro_context_anti":
        ...
        if toy_params["n_delay"] > 0:
            ...
            epoch_idxs = (
                ...
                post_context,  # Onset of post-context
                toy_params["phrase_length"]
                - np.abs(toy_params["n_delay"]),  # End of delay
            )
    ...
    # Filters targets by context
    targets = np.zeros((dataset_size, toy_params["phrase_length"], 1), dtype=np.int32)
    targets[:, -1, 0] = np.where(
        context_mask, targets_1[:, -1, 0], anti_targets_1[:, -1, 0]
    )
...
def enumerate_scores(toy_params):
    ...
    if n_unordered == 3:
        ...
        for score_idx in range(len(scores)):
            ...
            if score[0] <= (phrase_length / 2 - filter) or score[1] <= (
                phrase_length / 2 - filter
            ):
...
def n_scores(n_classes, phrase_length):
    ...
    return int(
        np.prod(
            np.array(
                [n + 1 + phrase_length for n in range(0, n_classes)], dtype=np.float64
            )
        )
        / np.prod(np.array([n + 1 for n in range(n_classes)], dtype=np.float64))
...
def check_word_bank_for_uniform_score_ordered(words):
    ...
    if len(words) == 3:
        ...
        if words != [
            ...
            "awful",
            "the",
        ]:  # Needs this exact ordering for now
            contradiction = True
    elif len(words) == 6:
        if words != [
            "awesome",
...
def score_to_phrase_ordered_general(score, score_map, toy_params):
    ...
    raw_count = index_to_score_unordered(
        score_map[score][phrase_idx], len(words) - 1, phrase_length
    )
